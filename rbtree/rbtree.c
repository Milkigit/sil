...
void insert_rebalance(struct sil_rb_path *path)
{
        ...
        /*
         * Left and right might actually be right and left, but we just
         * normalize directions to have to think only one variant.  Left is the
         * direction from parent to head.
         */
        int left;
        int right;
        /*
        ...
        assert(sil_rb_cld_is_black(ggpnt, sil_rb_get_direction(ggpnt, gpnt)));
...
static void delete_rebalance(struct sil_rb_path *path)
{
        ...
        if (sil_rb_cld_is_red(pnt, right)) {
                ...
                sil_rb_set_cld(gpnt, gdir, sleft, sil_rb_cld_get_color(gpnt, gdir));
                ...
                sil_rb_set_cld(gpnt, gdir, sibling, sil_rb_cld_get_color(gpnt, gdir));
